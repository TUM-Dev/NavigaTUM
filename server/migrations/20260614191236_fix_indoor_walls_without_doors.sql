-- A tile with no doors at the requested level has v_doors.geom = NULL (ST_Union over
-- zero rows). ST_Difference is STRICT, so it returned NULL and blanked the whole tile.
-- Coalesce to an empty geometry, which ST_Difference leaves the walls untouched.
  ...
  FROM (
    SELECT
      ST_AsMVTGeom(
        ST_Difference(
          v_walls.geom,
          COALESCE(v_doors.geom, 'GEOMETRYCOLLECTION EMPTY'::geometry)
        ),
