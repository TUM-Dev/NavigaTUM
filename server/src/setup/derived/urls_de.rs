...
pub async fn setup(pool: &PgPool) -> anyhow::Result<()> {
    ...
    let body = file_loader::load_file_or_download("urls_de.parquet", &cdn_url).await?;
...
async fn load_rows(pool: &PgPool, rows: &[RawUrl]) -> anyhow::Result<()> {
    ...
    sqlx::query!("TRUNCATE TABLE urls_de")
        .execute(&mut *tx)
        .await?;
    for r in rows {
        insert_row(&mut tx, r).await?;
    }
    sqlx::query!("ANALYZE urls_de").execute(&mut *tx).await?;
    tx.commit().await?;
    Ok(())
}
...
mod tests {
    ...
    // Legacy view's `link ->> 'url'` returned the bilingual object as text.
    // Pull the German side here for true parity.
    ...
    async fn urls_de_table_matches_jsonb_source() {
        ...
        load_rows(&pg.pool, &raw_rows)
            .await
            .expect("load urls_de from de-derived rows");

        let table_query = format!("SELECT key, url, text FROM urls_de{STABLE_ORDER}");
        let actual: Vec<UrlRow> = sqlx::query_as(&table_query)
            .fetch_all(&pg.pool)
            .await
            .expect("select from urls_de");
