...
def test_committed_opening_hours_csv_satisfies_schema() -> None:
    """The committed `opening_hours.csv` must satisfy `OpeningHoursSchema` and parse as OSM (drift gate)."""
...
def test_opening_hours_schema_rejects_macros(osm: str) -> None:
    """`lecture:`/`break:` macros are out of scope for this slice and must be rejected."""
    with pytest.raises(dy.exc.ValidationError):
...
def test_opening_hours_schema_rejects_non_http_source_url(url: str) -> None:
    """`source_url` must be an absolute http(s) URL so the webclient can link to provenance."""
    with pytest.raises(dy.exc.ValidationError):
...
def test_opening_hours_schema_rejects_non_iso_dates(field: str, bad_date: str) -> None:
    """Date fields must be `YYYY-MM-DD` so the Rust reader can parse them as plain dates."""
    with pytest.raises(dy.exc.ValidationError):
        OpeningHoursSchema.validate(_row_with(**{field: bad_date}))


def test_opening_hours_schema_rejects_inverted_validity_range() -> None:
    """`valid_until` before `valid_from` is a nonsensical window and must be rejected."""
    with pytest.raises(dy.exc.ValidationError):
...
def test_opening_hours_schema_accepts_bounded_validity_window() -> None:
    """A bounded `valid_from`/`valid_until` window (e.g. a semester) must validate (positive control)."""
