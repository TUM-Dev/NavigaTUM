...
def merge_tumonline_buildings(data):
    ...
    for b in buildings:
        ...
        b_data.setdefault("props", {}) \
              .setdefault("ids", {}) \
              .setdefault("b_id", b_id)
...
def merge_tumonline_rooms(data):
    ...
    for r in rooms:
        ...
        if "extended" in r and \
           "physikalische Eigenschaften" in r["extended"]:
        ...
        if len(arch_name_parts[0]) == 0:
            ...
            if len(alt_parts) >= 2:
                if alt_parts[0].lower() == arch_name_parts[0].lower():
                    ...
                    if arch_name_parts[0][:2] in {"R ", "L ", "M ", "N "} and \
                       alt_parts[0] == arch_name_parts[0].replace(" ", "", 2):
            ...
            if arch_name_parts[0][-1].isalpha() and roomcode_parts[2][-1].isalpha() and \
               arch_name_parts[0][-1].lower() != roomcode_parts[2][-1].lower():
                   ...
                   (an.endswith(r2_parts[2].lower()) and not an.endswith(r1_parts[2].lower())):
                    ...
                    if r1_parts[2][:min_len] == r2_parts[2][:min_len]:
                        if len(r1_parts[2]) > len(r2_parts[2]):
                            r["arch_name"] = arch_name_parts[0] + r1_parts[2][min_len:].lower() + \
                                             "@" + arch_name_parts[1]
                        else:
