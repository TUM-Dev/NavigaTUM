...
(function(){
    ...
    history.saveCurrentViewState = function() {
        if (navigatum.router
            ...
            var state_index = history.lastStateIndex === null
                              ? history.stateIndex : history.lastStateIndex;
        
            var cloned = navigatum.cloneState(
                navigatum.router.currentRoute.matched[0].instances.default.state
            );
            history.states[state_index][0][stateDataSymbol] = cloned;
        }
    }
    ...
    addEventListener("popstate",function(e){
        ...
        var eventObject= {};
        var newStateIndex =  e.state != null && e.state[stateIndexSymbol] !== undefined
                             ? e.state[stateIndexSymbol] : 0;
        eventObject.from = history.states[history.stateIndex];
        eventObject.to   = newStateIndex in history.states
                           ? history.states[newStateIndex] : null;
        ...
        history.stateIndex = e.state!=null && e.state[stateIndexSymbol] !== undefined
                             ? e.state[stateIndexSymbol] : 0;//-1;
    });
})();
