...
<template>
  <div class="space-y-3">
    <div>
      <label class="text-zinc-600 dark:text-zinc-300 mb-1 block text-xs font-medium" for="add-room-arch-name">
        {{ t("arch_name") }} <span class="text-red-700 dark:text-red-200">*</span>
      </label>
      <input
    ...
    <div>
      <span class="text-zinc-600 dark:text-zinc-300 mb-1 block text-xs font-medium">{{ t("usage_id") }} <span class="text-red-700 dark:text-red-200">*</span></span>
      <Combobox v-model="selectedUsage" :nullable="true" by="usage_id">
        <div class="relative">
          ...
          <Transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100" leave-to-class="opacity-0">
            ...
            >
              <p v-if="filteredUsages.length === 0" class="text-zinc-500 dark:text-zinc-400 px-3 py-2 text-sm">
                {{ t("usage_no_results") }}
              </p>
              <ComboboxOption
    ...
    <details class="border-zinc-300 dark:border-zinc-600 rounded border px-3 py-2">
      ...
      <div class="mt-3 space-y-3">
        <div>
          <span class="text-zinc-600 dark:text-zinc-300 mb-1 block text-xs font-medium">{{ t("floor.legend") }}</span>
          <p class="text-zinc-500 dark:text-zinc-400 mb-2 text-xs">{{ t("floor.help") }}</p>
        ...
        <div>
          <span class="text-zinc-600 dark:text-zinc-300 mb-1 block text-xs font-medium">{{ t("seats") }}</span>
