...
export class FloorControl extends Evented implements IControl {
	...
	public updateFloors(overlays: OverlayMap): void {
		...
		if (visibleI === null) {
			...
			this._setActiveFloor(
				visibleI,
				overlays.available[visibleI]?.floor ?? "EG",
			);
			this.fire("floor-changed", {
	...
	private _recalculateLayout(n: number): void {
		...
		const mapHeight =
			document.getElementById("interactive-map")?.clientHeight || 0;
		const topCtrlHeight =
			document.querySelector(".maplibregl-ctrl-top-left")?.clientHeight || 0;
		const bottomCtrlHeight =
			document.querySelector(".maplibregl-ctrl-bottom-left")?.clientHeight || 0;
		const floorCtrlHeight =
			document.querySelector(".floor-ctrl")?.clientHeight || 0;
