...
effect(() => {
  ...
  indoorMap.value.drawRoute(data.value.legs[0].shape, data.value.legs[0].maneuvers);
});

function setBoundingBoxFromIndex(from_shape_index: number, to_shape_index: number) {
  if (!data.value) return;

  let coords = data.value.legs[0].shape.slice(from_shape_index, to_shape_index);
...
<template>
  <div class="flex min-h-[calc(100vh-150px)] flex-row">
      ...
      <div class="overflow-auto">
        <div v-if="status === 'success' && !!data">
          <div v-for="(l, i) in data.legs" :key="i" class="gap-1">
            <p class="text-zinc-500 font-semibold mt-3 flex items-center gap-5 pb-4">
            ...
            <div
              ...
              :key="j"
              class="py-1 cursor-pointer group"
              @click="setBoundingBoxFromIndex(m.begin_shape_index, m.end_shape_index)"
            >
              <div class="flex flex-row items-center bg-zinc-200 py-1 gap-3 p-2 rounded-md group-hover:bg-zinc-300">
                <RoutingManeuverIcon :type="m.type" />
                <div>
                  <div class="text-zinc-900">{{ m.instruction }}</div>
                </div>
              </div>
              <small class="text-zinc-500" v-if="m.length_meters">{{ t("meters", m.length_meters) }}</small>
            </div>
          </div>
        </div>
        <Toast id="nav-error" level="error" v-else>
    ...
    <div class="grow">
      <IndoorMap type="room" :coords="{ lat: 0, lon: 0, source: 'navigatum' }" ref="indoorMap" />
    </div>
  </div>
</template>
